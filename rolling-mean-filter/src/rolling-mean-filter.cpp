...
int main(int argc, char* argv[])
{
  // parse command line arguments
    ...
    size_t buffer_count = IMAGE_SIZE;
    size_t iterations = 1;
  #else
    size_t reps = 200;
...
template<typename T>
void DoRooflineAnalysis(queue& q, size_t buffer_count, size_t iterations, size_t threads)
{
  ...
  for (size_t i = 0; i < bw_test_iterations; i++)
  {
    ...
    ConsumerFunction(tmp_in[0], tmp_out[0], buffer_count, half_threads, 0);
    end = high_resolution_clock::now();
    delta_ms = end - start;
    ...
    ConsumerFunction(tmp_out[1], tmp_in[1], buffer_count, half_threads, 0);
