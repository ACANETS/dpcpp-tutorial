                       ...
                       high_resolution_clock::time_point& end)
{
  ...
  std::cout << "Half-threads." << std::endl;
  ...
  std::cout << "Init MyStreamer..." << std::endl;
  ...
  std::cout << "Submit Kernel..." << std::endl;
  ...
  std::cout << "Init producer thread..." << std::endl;
  ...
  std::cout << "Joining producer thread..." << std::endl;
  ...
  std::cout << "Sync with HostStreamer" << std::endl;
  MyStreamer::Sync();
  ...
  std::cout << "Wait for kernel event..." << std::endl;
  kernel_event.wait();

  end = high_resolution_clock::now();

  // destroy the streamer data structures
  std::cout << "Destroying streamer..." << std::endl;
  MyStreamer::destroy();
}
               ...
               size_t iterations, size_t threads)
{
  ...
  static const char* outputImagePath = "./output/cat.bmp";
  ...
  std::vector<float*> in_stream(total_count);
  std::vector<float*> out_stream(total_count);

  //
  // Read in the image
  //
  std::cout << "Reading image..." << std::endl;
  in_stream[0] = readBmpFloat(inputImagePath, &imageRows, &imageCols);
  inputImage = readBmpFloat(inputImagePath, &imageRows, &imageCols);
  std::cout << "Read image..." << std::endl;

  if (debug)
  {
    std::cout << "imageRows=" << imageRows << ", imageCols=" << imageCols << std::endl;
  }
  ...
  std::cout << "Set memory for outputImage" << std::endl;
  out_stream[0] = outputImage;

  std::cout << "Finished allocating streams..." << std::endl;
  ...
  for (size_t i = 0; i < iterations; i++)
  {
    ...
    outputImage = out_stream[0];
    std::cout << "Output image saved as " << outputImagePath << std::endl;
    writeBmpFloat(outputImage, outputImagePath, imageRows, imageCols, outputImagePath);
