...
template<typename T>
void ConsumerFunction(T* dst, const T* src, size_t count, int num_threads, size_t rep)
{
  const char* inputImagePath = "./input/cat.bmp";
  const std::string output_filename = "./output/cat_" + std::to_string(rep) + ".bmp";
  const char* outputImagePath = output_filename.c_str();
  ...
  writeBmpFloat(*src, outputImagePath, imageRows, imageCols, inputImagePath);
